    ...
    grid.innerHTML = DRINKS[cat].map(d => `
      <div class="drink-card" data-id="${d.id}" data-cat="${cat}" tabindex="0" role="button" aria-label="Customize ${d.name}">
        ...
        <div class="card-body">
          ...
          ${d.seasonal || d.tag ? `<div class="card-badges">${d.seasonal ? '<div class="tag-seasonal">Seasonal</div>' : ''}${d.tag ? `<div class="tag-seasonal">${d.tag}</div>` : ''}</div>` : ''}
